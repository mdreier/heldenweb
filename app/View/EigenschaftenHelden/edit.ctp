<div class="eigenschaftenHelden form">
<?php echo $this->Form->create('EigenschaftenHeld');?>
	<fieldset>
		<legend><?php echo __('Edit Eigenschaften Held'); ?></legend>
	<?php
		echo $this->Form->input('id');
		echo $this->Form->input('held_id');
		echo $this->Form->input('eigenschaft_id');
		echo $this->Form->input('wert');
	?>
	</fieldset>
<?php echo $this->Form->end(__('Submit'));?>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>

		<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $this->Form->value('EigenschaftenHeld.id')), null, __('Are you sure you want to delete # %s?', $this->Form->value('EigenschaftenHeld.id'))); ?></li>
		<li><?php echo $this->Html->link(__('List Eigenschaften Helden'), array('action' => 'index'));?></li>
		<li><?php echo $this->Html->link(__('List Helden'), array('controller' => 'helden', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Held'), array('controller' => 'helden', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Eigenschaften'), array('controller' => 'eigenschaften', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Eigenschaft'), array('controller' => 'eigenschaften', 'action' => 'add')); ?> </li>
	</ul>
</div>
